    ...
    def _on_pose(self, msg):
        for e in msg.pose:
            if e.name == self.vessel:
                ...
                self.x, self.y = e.position.y, e.position.x   # NED North = ENU.y, NED East = ENU.x
                q = e.orientation
                yaw_enu = math.atan2(2 * (q.w * q.z + q.x * q.y),
                ...
                if self._prev_yaw is not None and self._prev_t is not None and t > self._prev_t:
                    self.r = math.atan2(math.sin(yaw - self._prev_yaw),
                                        math.cos(yaw - self._prev_yaw)) / (t - self._prev_t)
