...
**Remaining:** Windows. The 4090 lives there and it is where a demonstration would
be shown; nothing about that path has been exercised yet.
...
The cheapest of the three and the best demonstration value: a ROS node publishing
the same `vessel_cmd_array` JSON the helmsman publishes, so **nothing downstream
changes**. Sailing the boat by hand next to an algorithm sailing its own is the
clearest way to show what the simulator is.
